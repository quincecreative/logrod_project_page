...
//ENG
let coordinates = ['-1.71,0.20,2.33', '3.05,0.23,-1.67', '2.21,0.22,-0.83', '2.53,0.01,-1.08', '-1.35,0.30,10.37', '-2.12,0.01,10.98', '-26.53,0.09,33.74', '21.31,0.49,12.18', '-13.76,0.01,23.03', '-3.23,0.19,10.19']
...
//SWITCH
let mainContainerMapTL = gsap.fromTo(mainContainerMap, { top: -99 + "dvh", }, { top: 0, duration: 1, delay: 0 })
mainContainerMapTL.pause()

let mainContainerProjectsTL = gsap.fromTo(mainContainerProjects, { top: -100 + "dvh", }, { top: 0, duration: 1, delay: 0 })
...
let projectsTransparentImgArray = [
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639cef0142dbeb7f85318f1_11.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639ceef5130e047f54cca18_22.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639cef07bac6326f17e5fa7_33.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639cef02bae31d6bc47e615_44.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639ceef66343b534fa5c02f_55.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639cef0bf79e9627e58f86a_66.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639cef62d236cf9e077c622_77.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639cef0142dbeb7f85318f1_11.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639ceef5130e047f54cca18_22.png",
    "https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6639cef07bac6326f17e5fa7_33.png",
]
...
for (let i = 0; i < numOfTransperentBckgImgs; i++) {
    ...
    if (i == 1) transparentBckgImg.style.scale = "0.8"
...
let bigDraggable = Draggable.create(projBack, {
    ...
    onDrag: function (e) {
        ...
        if (screen.width > screen.height) cursorHandle(x, y)
...
mainContainerMap.appendChild(instructionsContainer)
instructionsContainer.classList.add("instructionsContainer")

let moveContainer = document.createElement("div")
instructionsContainer.appendChild(moveContainer)
moveContainer.classList.add("moveContainer")

let moveText = document.createElement("div")
moveContainer.appendChild(moveText)
moveText.classList.add("moveText")
moveText.innerHTML = "MOVE"

let moveIcon = document.createElement("div")
moveContainer.appendChild(moveIcon)
moveIcon.classList.add("moveIcon")
moveIcon.style.background = `url(https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6626c8b49d117556857f683d_moveIcon2.png)`;
moveIcon.style.backgroundSize = 'cover';
moveIcon.style.backgroundPosition = 'center';

let zoomContainer = document.createElement("div")
instructionsContainer.appendChild(zoomContainer)
zoomContainer.classList.add("zoomContainer")

let zoomText = document.createElement("div")
zoomContainer.appendChild(zoomText)
zoomText.classList.add("zoomText")
...
zoomIcon.style.background = `url(https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6626c8b4bf098ea784dbdca2_zoomIcon2.png)`;
zoomIcon.style.backgroundSize = 'cover';
zoomIcon.style.backgroundPosition = 'center';

let rotateContainer = document.createElement("div")
instructionsContainer.appendChild(rotateContainer)
rotateContainer.classList.add("rotateContainer")

let rotateText = document.createElement("div")
rotateContainer.appendChild(rotateText)
rotateText.classList.add("rotateText")
...
rotateIcon.style.background = `url(https://uploads-ssl.webflow.com/647095916f065a8bf193de4e/6626c8b4c2ceccdaefae439e_rotateIcon2.png)`;
rotateIcon.style.backgroundSize = 'cover';
rotateIcon.style.backgroundPosition = 'center';
...
var createScene = async function () {
    ...
    function cameraTargetAnimation(x, y, z, newCameraRadius) {
        ...
        map.animations.push(animationCameraTarget);
        //call animation
        scene.beginAnimation(map, 0, 120, false);
    ...
    if (screen.width < screen.height) {
        camera.setPosition(new BABYLON.Vector3(-3.99, 30, 0))
    }

    camera.minZ = 0.1;
    camera.lowerRadiusLimit = 5;
    camera.upperRadiusLimit = 30;//80
    ...
    cameraTarget.position = new BABYLON.Vector3(1, -0.03, 5);
    ...
    let result = await Promise.all([
        BABYLON.SceneLoader.ImportMeshAsync(
            "",
            // "https://raw.githubusercontent.com/quincecreative/logrodProjectPage/gh-pages/Hamburg.glb",
            // "https://work.quincemedia.com/logrod-project-page/Hamburg.glb",
            "Hamburg.glb",
            null,
            scene,
            (evt) => {
    ...
    mapRootClone.position = new BABYLON.Vector3(0, 0, 56.1)
    mapRootClone.scaling = new BABYLON.Vector3(1, 1, 1)

    let mapRootClone2 = mapaRoot.clone()
    mapRootClone2.position = new BABYLON.Vector3(0, 0, -56.1)
    mapRootClone2.scaling = new BABYLON.Vector3(1, 1, 1)

    let mapRootClone3 = mapaRoot.clone()
    mapRootClone3.position = new BABYLON.Vector3(-94.5, 0, -56.1)
    mapRootClone3.scaling = new BABYLON.Vector3(1, 1, 1)

    let mapRootClone4 = mapaRoot.clone()
    mapRootClone4.position = new BABYLON.Vector3(-94.5, 0, 0)
    mapRootClone4.scaling = new BABYLON.Vector3(1, 1, 1)

    let mapRootClone5 = mapaRoot.clone()
    mapRootClone5.position = new BABYLON.Vector3(-94.5, 0, 56.1)
    mapRootClone5.scaling = new BABYLON.Vector3(1, 1, 1)

    let mapRootClone6 = mapaRoot.clone()
    mapRootClone6.position = new BABYLON.Vector3(94.5, 0, 0)
    mapRootClone6.scaling = new BABYLON.Vector3(1, 1, 1)

    let mapRootClone7 = mapaRoot.clone()
    mapRootClone7.position = new BABYLON.Vector3(94.5, 0, 56.1)
    mapRootClone7.scaling = new BABYLON.Vector3(1, 1, 1)

    let mapRootClone8 = mapaRoot.clone()
    mapRootClone8.position = new BABYLON.Vector3(94.5, 0, -56.1)
    mapRootClone8.scaling = new BABYLON.Vector3(1, 1, 1)

    let map = BABYLON.MeshBuilder.CreateGround("map", { width: 90, height: 110 }, scene, false);
    map.position.y = 1.5;
    map.position.z = 30
    map.visibility = 0

    map.addChild(mapaRoot)
    map.addChild(mapRootClone)
    map.addChild(mapRootClone2)
    map.addChild(mapRootClone3)
    map.addChild(mapRootClone4)
    map.addChild(mapRootClone5)
    map.addChild(mapRootClone6)
    map.addChild(mapRootClone7)
    map.addChild(mapRootClone8)
    ...
    let adjustRotating = 0;
    let adjustScaling = 0;
    let adjustYmovment = 0;
    let zooming = false
    let rotating = false
    // let panning = false
    ...
    if (screen.width < screen.height) {
        mc.on("pinchstart rotatestart", function (e) {
            ...
            adjustRotating = e.rotation
            adjustScaling = e.scale
            adjustYmovment = e.deltaY
        ...
        mc.on("pinchmove rotatemove", function (e) {
            if (((e.rotation - adjustRotating) < -10 || (e.rotation - adjustRotating) > 10) && !zooming) {
                rotating = true
            }
            if (adjustRotation < e.rotation && !zooming && rotating) {
                // switcher.innerHTML = `${e.rotation - adjustRotating}`
                if ((e.rotation - adjustRotation) > 0.15) {
                    camera.alpha += 0.03
                }
            }
            if (adjustRotation > e.rotation && !zooming && rotating) {
            ...
            if (adjustYmovment > e.deltaY && !zooming) {
                camera.beta += 0.03
            }
            if (adjustYmovment < e.deltaY && !zooming) {
                camera.beta -= 0.03
            ...
            // switcher.innerHTML = `${zooming} ${rotating}`

            if (adjustScaling > e.scale && !rotating) {
                if ((e.scale - adjustScaling) < -0.1) {
                    camera.angularSensibilityX = 2000;
                    camera.angularSensibilityY = 3000;
                    zooming = true
                }
        ...
        switch (pointerInfo.type) {
            ...
            case BABYLON.PointerEventTypes.POINTERMOVE:
                if (!devMode) {
                    ...
                    // console.log(map.position.z)
                    if (map.position.x < -15) {
                        map.position.x = -15;
                    }
                    if (map.position.x > 13) {
                        map.position.x = 13;
                    }
                    if (map.position.z < 18) {
                        map.position.z = 18;
                    }
                    if (map.position.z > 42) {
                        map.position.z = 42;
                    }
                }
                break;
            case BABYLON.PointerEventTypes.POINTERPICK:
                if (screen.width < screen.height && !anotationClicked) {
                ...
                if (!devMode) {
                    ...
                    if (screen.width < screen.height) {
                        ...
                        if (annotationForMobOpened[0] !== i) {
                            anotationClicked = true
                            anotationOpen(i)
                            annotationForMobOpened = [i]
                            setTimeout(() => {
                                anotationClicked = false
                            }, 500);
    ...
    toMapCenter.onclick = () => {
        ...
        if (screen.width > screen.height) {
            cameraTargetAnimation(0, 1.5, 30, 20, true)
        } else {
            cameraTargetAnimation(0, 1.5, 30, 30, true)
        }
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === 'v') {
            if (devMode) {
                ...
                // devMode = true;
                // map.position = new BABYLON.Vector3(0, 1.5, 30)
            }
        }
        if (e.key === 'c') {
            if (cameraOn == 0) {
